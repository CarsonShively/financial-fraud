...
def main():
    ...
    # Pick a RANDOM dest from those present in the dataset.
    # (Group by ensures uniqueness; RANDOM() makes selection unbiased-ish.)
    dest_id = con.execute(
        """
        SELECT name_dest
        FROM read_parquet(?)
    ...
    if mismatches:
        ...
        print(f"❌ Parity mismatches for dest={dest_id} (first 50):")
        print(out.head(50).to_string(index=False))
        print(f"\nTotal mismatches: {len(mismatches)}")
    else:
        print(f"✅ Parity passed for RANDOM dest={dest_id} on {len(sub)} rows")
